...
## Script that demonstrates the generation of a mesh using a compute shader.
...
func _create_planet_mesh() -> Array:
	...
	var shader_file := load("res://mesh_tester/shaders/compute_shader.glsl")
	...
	# Create two uniforms to assign the buffers to the rendering device
	...
	# Submit to the GPU and wait for sync
